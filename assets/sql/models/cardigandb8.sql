-- MySQL Script generated by MySQL Workbench
-- Sun Mar  1 23:24:25 2020
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';

-- -----------------------------------------------------
-- Schema mydb
-- -----------------------------------------------------
-- -----------------------------------------------------
-- Schema cardigandb
-- -----------------------------------------------------

-- -----------------------------------------------------
-- Schema cardigandb
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `cardigandb` DEFAULT CHARACTER SET utf8 ;
USE `cardigandb` ;

-- -----------------------------------------------------
-- Table `cardigandb`.`attribute`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cardigandb`.`attribute` (
  `idAttribute` INT(11) NOT NULL AUTO_INCREMENT,
  `Name` VARCHAR(45) NOT NULL,
  `Value_Type` ENUM('BOOL', 'INT', 'DECIMAL', 'TEXT', 'VARCHAR', 'TIMESTAMP', 'DATE') NOT NULL,
  `Units` VARCHAR(45) NULL,
  PRIMARY KEY (`idAttribute`),
  UNIQUE INDEX `idAttribute_UNIQUE` (`idAttribute` ASC) VISIBLE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;


-- -----------------------------------------------------
-- Table `cardigandb`.`data_type`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cardigandb`.`data_type` (
  `idData_Type` INT(11) NOT NULL AUTO_INCREMENT,
  `Type` ENUM('Clinical', 'Gait') NOT NULL,
  `Subtype` ENUM('General', 'Activity', 'Biochemical Levels', 'Diet', 'Up', 'Down') NOT NULL,
  `Walk_Type` ENUM('6 Minute', '10 Metre', '') NULL,
  `Weekly` TINYINT(1) NOT NULL,
  PRIMARY KEY (`idData_Type`),
  UNIQUE INDEX `idData Type_UNIQUE` (`idData_Type` ASC) VISIBLE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;


-- -----------------------------------------------------
-- Table `cardigandb`.`entity`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cardigandb`.`entity` (
  `idEntity` INT(11) NOT NULL AUTO_INCREMENT,
  `idDataType` INT(11) NOT NULL,
  `Name` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`idEntity`),
  UNIQUE INDEX `Entity_ID_UNIQUE` (`idEntity` ASC) VISIBLE,
  INDEX `fk_Entity_Data Type1_idx` (`idDataType` ASC) VISIBLE,
  INDEX `idDataType` (`idDataType` ASC) VISIBLE,
  CONSTRAINT `fk_Entity_Data Type1`
    FOREIGN KEY (`idDataType`)
    REFERENCES `cardigandb`.`data_type` (`idData_Type`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;


-- -----------------------------------------------------
-- Table `cardigandb`.`user`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cardigandb`.`user` (
  `ID` INT NOT NULL AUTO_INCREMENT,
  `Password` VARCHAR(45) NOT NULL,
  `Salt` VARCHAR(45) NOT NULL,
  `User_Type` ENUM('Unverified', 'Verified', 'Admin') NOT NULL DEFAULT 'Unverified',
  `Forename` VARCHAR(45) NOT NULL,
  `Surname` VARCHAR(45) NOT NULL,
  `Email` VARCHAR(80) NOT NULL,
  `Account_Created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP(),
  `Last_Updated` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
  `Last_Logged_In` TIMESTAMP NULL,
  PRIMARY KEY (`ID`),
  UNIQUE INDEX `Username_UNIQUE` (`ID` ASC) VISIBLE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;


-- -----------------------------------------------------
-- Table `cardigandb`.`visitation`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cardigandb`.`visitation` (
  `idVisitation` INT(11) NOT NULL,
  `Date_Start` DATE NOT NULL,
  `Date_End` DATE NOT NULL,
  `Preconsultation` TINYINT(1) NOT NULL DEFAULT 0,
  PRIMARY KEY (`idVisitation`),
  UNIQUE INDEX `idVisitation_UNIQUE` (`idVisitation` ASC) VISIBLE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;


-- -----------------------------------------------------
-- Table `cardigandb`.`Patient`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cardigandb`.`Patient` (
  `idPatient` INT(11) NOT NULL,
  `Patient_ID` VARCHAR(5) NOT NULL,
  PRIMARY KEY (`idPatient`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `cardigandb`.`value`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cardigandb`.`value` (
  `idValue` INT(11) NOT NULL,
  `idEntity` INT(11) NOT NULL,
  `idAttribute` INT(11) NOT NULL,
  `idVisitation` INT(11) NOT NULL,
  `idPatient` INT(11) NOT NULL,
  PRIMARY KEY (`idValue`),
  UNIQUE INDEX `idValue_UNIQUE` (`idValue` ASC) VISIBLE,
  INDEX `fk_value_visitation1_idx` (`idVisitation` ASC) VISIBLE,
  INDEX `fk_value_entity1_idx` (`idEntity` ASC) VISIBLE,
  INDEX `fk_value_attribute1_idx` (`idAttribute` ASC) VISIBLE,
  INDEX `fk_value_Patient1_idx` (`idPatient` ASC) VISIBLE,
  CONSTRAINT `fk_value_visitation1`
    FOREIGN KEY (`idVisitation`)
    REFERENCES `cardigandb`.`visitation` (`idVisitation`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_value_entity1`
    FOREIGN KEY (`idEntity`)
    REFERENCES `cardigandb`.`entity` (`idEntity`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_value_attribute1`
    FOREIGN KEY (`idAttribute`)
    REFERENCES `cardigandb`.`attribute` (`idAttribute`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_value_Patient1`
    FOREIGN KEY (`idPatient`)
    REFERENCES `cardigandb`.`Patient` (`idPatient`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `cardigandb`.`value_boolean`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cardigandb`.`value_boolean` (
  `idValue_Boolean` INT(11) NOT NULL AUTO_INCREMENT,
  `idValue` INT(11) NOT NULL,
  `Value` TINYINT(1) NOT NULL,
  PRIMARY KEY (`idValue_Boolean`),
  UNIQUE INDEX `idValue_Boolean_UNIQUE` (`idValue_Boolean` ASC) VISIBLE,
  INDEX `fk_value_boolean_value1_idx` (`idValue` ASC) VISIBLE,
  CONSTRAINT `fk_value_boolean_value1`
    FOREIGN KEY (`idValue`)
    REFERENCES `cardigandb`.`value` (`idValue`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;


-- -----------------------------------------------------
-- Table `cardigandb`.`value_decimal`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cardigandb`.`value_decimal` (
  `idValue_Decimal` INT(11) NOT NULL AUTO_INCREMENT,
  `idValue` INT(11) NOT NULL,
  `Value` DECIMAL(10,3) NOT NULL,
  PRIMARY KEY (`idValue_Decimal`),
  UNIQUE INDEX `idValue_Decimal_UNIQUE` (`idValue_Decimal` ASC) VISIBLE,
  INDEX `fk_value_decimal_value1_idx` (`idValue` ASC) VISIBLE,
  CONSTRAINT `fk_value_decimal_value1`
    FOREIGN KEY (`idValue`)
    REFERENCES `cardigandb`.`value` (`idValue`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;


-- -----------------------------------------------------
-- Table `cardigandb`.`value_integer`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cardigandb`.`value_integer` (
  `idValue_Integer` INT(11) NOT NULL AUTO_INCREMENT,
  `idValue` INT(11) NOT NULL,
  `Value` INT(11) NOT NULL,
  PRIMARY KEY (`idValue_Integer`),
  UNIQUE INDEX `idValue_Integer_UNIQUE` (`idValue_Integer` ASC) VISIBLE,
  INDEX `fk_value_integer_value1_idx` (`idValue` ASC) VISIBLE,
  CONSTRAINT `fk_value_integer_value1`
    FOREIGN KEY (`idValue`)
    REFERENCES `cardigandb`.`value` (`idValue`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;


-- -----------------------------------------------------
-- Table `cardigandb`.`value_text`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cardigandb`.`value_text` (
  `idValue_Text` INT(11) NOT NULL AUTO_INCREMENT,
  `idValue` INT(11) NOT NULL,
  `Value` TEXT NOT NULL,
  PRIMARY KEY (`idValue_Text`),
  UNIQUE INDEX `idValue_Text_UNIQUE` (`idValue_Text` ASC) VISIBLE,
  INDEX `fk_value_text_value1_idx` (`idValue` ASC) VISIBLE,
  CONSTRAINT `fk_value_text_value1`
    FOREIGN KEY (`idValue`)
    REFERENCES `cardigandb`.`value` (`idValue`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;


-- -----------------------------------------------------
-- Table `cardigandb`.`value_varchar`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cardigandb`.`value_varchar` (
  `idValue_Varchar` INT(11) NOT NULL AUTO_INCREMENT,
  `idValue` INT(11) NOT NULL,
  `Value` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`idValue_Varchar`),
  UNIQUE INDEX `idValue_Varchar_UNIQUE` (`idValue_Varchar` ASC) VISIBLE,
  INDEX `fk_value_varchar_value1_idx` (`idValue` ASC) VISIBLE,
  CONSTRAINT `fk_value_varchar_value1`
    FOREIGN KEY (`idValue`)
    REFERENCES `cardigandb`.`value` (`idValue`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;


-- -----------------------------------------------------
-- Table `cardigandb`.`value_timestamp`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cardigandb`.`value_timestamp` (
  `idValue_Timestamp` INT(11) NOT NULL,
  `value_idValue` INT(11) NOT NULL,
  `Value` TIMESTAMP NOT NULL,
  PRIMARY KEY (`idValue_Timestamp`),
  UNIQUE INDEX `idValue_Timestamp_UNIQUE` (`idValue_Timestamp` ASC) VISIBLE,
  INDEX `fk_value_timestamp_value1_idx` (`value_idValue` ASC) VISIBLE,
  CONSTRAINT `fk_value_timestamp_value1`
    FOREIGN KEY (`value_idValue`)
    REFERENCES `cardigandb`.`value` (`idValue`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `cardigandb`.`value_date`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cardigandb`.`value_date` (
  `idValue_Text` INT(11) NOT NULL AUTO_INCREMENT,
  `idValue` INT(11) NOT NULL,
  `Value` DATE NOT NULL,
  PRIMARY KEY (`idValue_Text`),
  UNIQUE INDEX `idValue_Text_UNIQUE` (`idValue_Text` ASC) VISIBLE,
  INDEX `fk_value_text_value1_idx` (`idValue` ASC) VISIBLE,
  CONSTRAINT `fk_value_text_value10`
    FOREIGN KEY (`idValue`)
    REFERENCES `cardigandb`.`value` (`idValue`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;
